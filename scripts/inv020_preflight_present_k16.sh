...
python train.py \
  --resource-preflight \
  --provenance-out logs/preflight/inv020_fixed_run60_present_k16.json \
  --data ego4d \
  ...
  --lambda-recon-pred 0 \
  --lr-coarse-flow 1e-4 \
  --checkpoint-dir /workspace/ckpt/inv020_fixed_run60_present_k16 \
  --log-every 50 \
  --diag-every 500 \
  --wandb-entity smahalanobis-uc-davis \
  --wandb-project hjepa-vwm \
  --wandb-group inv020_fixed_run60_source_comparison \
  --wandb-name "Investigation 20 · Fixed Run 60 flow source · k=16 present"
