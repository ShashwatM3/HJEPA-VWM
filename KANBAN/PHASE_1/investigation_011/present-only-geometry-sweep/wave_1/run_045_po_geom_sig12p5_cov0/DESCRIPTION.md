# Run 045 - `po_geom_sig12p5_cov0`

**Current W&B run name:** `Investigation 11 · Present geometry · Isotropy 12.5, no covariance`

<!-- AUTO-GENERATED-WANDB-KANBAN -->

**Investigation:** [investigation_011](../../../)  
**W&B:** `76d6o8d2` - https://wandb.ai/smahalanobis-uc-davis/hjepa-vwm/runs/76d6o8d2  
**State:** `finished`  
**Created:** 2026-07-01T20:55:02Z  
**Last history step:** 14950  
**Runtime in W&B export:** 5.94h  
...
Present-only geometry sweep member: sigreg=12.5, cov=0.
...
| `agc_enabled` | true |
| `checkpoint_dir` | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig12p5_cov0 |
...
|---|---:|---:|
| `lambda_cov` | 0.003 | 0 |
| `lambda_sigreg` | 10 | 12.5 |
| `checkpoint_dir` | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig10_cov0p003 | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig12p5_cov0 |

## Chronological Linkage

- Previous W&B run: Run 044 [`po_geom_sig10_cov0p003`](../run_044_po_geom_sig10_cov0p003/).
- Next W&B run: Run 046 [`po_geom_sig10_cov0`](../run_046_po_geom_sig10_cov0/).
...
|---|---|
| W&B run files | `artifact/3086168560/wandb_manifest.json` (0 bytes), `config.yaml` (6398 bytes), `output.log` (234418 bytes), `requirements.txt` (3821 bytes), `wandb-metadata.json` (2636 bytes), `wandb-summary.json` (1157 bytes) |
| Logged artifacts | `run-76d6o8d2-history:v0` (wandb-history) |
| API/group fetch caveats | none |

## Original Notes Preserved

**Command** (present-only geometry-sweep member; grid point lambda_sigreg=12.5 x lambda_cov=0.0 over the fixed-position/cosine present-only bottleneck; anchor/control = run 041 sig5/cov0):
...
python train.py --data ssv2 --steps 15000 --present-recon-only --recon-loss-mode cosine --lambda-recon 0.05 --lambda-var 0.5 --sigreg-warmup-steps 2000 --decoder-dim 512 --decoder-blocks 4 --n-c 32 --horizon-k 12 --log-every 50 --diag-every 500 \
  --lambda-sigreg 12.5 --lambda-cov 0.0 \
  --checkpoint-dir /workspace/ckpt/inv011_present_only_geometry_sweep/run_045_po_geom_sig12p5_cov0
```
