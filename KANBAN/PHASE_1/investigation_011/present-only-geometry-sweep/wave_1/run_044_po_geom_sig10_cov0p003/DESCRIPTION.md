# Run 044 - `po_geom_sig10_cov0p003`

**Current W&B run name:** `Investigation 11 · Present geometry · Isotropy 10, covariance 0.003`

<!-- AUTO-GENERATED-WANDB-KANBAN -->

**Investigation:** [investigation_011](../../../)  
**W&B:** `5xockdbh` - https://wandb.ai/smahalanobis-uc-davis/hjepa-vwm/runs/5xockdbh  
**State:** `finished`  
**Created:** 2026-07-01T20:55:02Z  
**Last history step:** 14950  
**Runtime in W&B export:** 5.96h  
...
Present-only geometry sweep member: sigreg=10, cov=0.003.
...
| `agc_enabled` | true |
| `checkpoint_dir` | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig10_cov0p003 |
...
|---|---:|---:|
| `lambda_sigreg` | 5 | 10 |
| `checkpoint_dir` | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig5_cov0p003 | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig10_cov0p003 |

## Chronological Linkage

- Previous W&B run: Run 043 [`po_geom_sig5_cov0p003`](../run_043_po_geom_sig5_cov0p003/).
- Next W&B run: Run 045 [`po_geom_sig12p5_cov0`](../run_045_po_geom_sig12p5_cov0/).
...
|---|---|
| W&B run files | `artifact/3086168576/wandb_manifest.json` (0 bytes), `config.yaml` (6406 bytes), `output.log` (234686 bytes), `requirements.txt` (3821 bytes), `wandb-metadata.json` (2640 bytes), `wandb-summary.json` (1162 bytes) |
| Logged artifacts | `run-5xockdbh-history:v0` (wandb-history) |
| API/group fetch caveats | none |

## Original Notes Preserved

**Command** (present-only geometry-sweep member; grid point lambda_sigreg=10.0 x lambda_cov=0.003 over the fixed-position/cosine present-only bottleneck; anchor/control = run 041 sig5/cov0):
...
python train.py --data ssv2 --steps 15000 --present-recon-only --recon-loss-mode cosine --lambda-recon 0.05 --lambda-var 0.5 --sigreg-warmup-steps 2000 --decoder-dim 512 --decoder-blocks 4 --n-c 32 --horizon-k 12 --log-every 50 --diag-every 500 \
  --lambda-sigreg 10.0 --lambda-cov 0.003 \
  --checkpoint-dir /workspace/ckpt/inv011_present_only_geometry_sweep/run_044_po_geom_sig10_cov0p003
```
