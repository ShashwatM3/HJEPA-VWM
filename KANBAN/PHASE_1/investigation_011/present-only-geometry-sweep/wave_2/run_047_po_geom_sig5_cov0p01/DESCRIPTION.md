# Run 047 - `po_geom_sig5_cov0p01`

**Current W&B run name:** `Investigation 11 · Present geometry · Isotropy 5, covariance 0.01`

<!-- AUTO-GENERATED-WANDB-KANBAN -->

**Investigation:** [investigation_011](../../../)  
**W&B:** `az60m6mx` - https://wandb.ai/smahalanobis-uc-davis/hjepa-vwm/runs/az60m6mx  
**State:** `crashed`  
**Created:** 2026-07-02T03:48:45Z  
**Last history step:** 14150  
...
Present-only geometry sweep wave 2 member: sigreg=5, cov=0.01.
...
| `agc_enabled` | true |
| `checkpoint_dir` | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig5_cov0p01 |
...
| `lambda_cov` | 0 | 0.01 |
| `lambda_sigreg` | 10 | 5 |
| `checkpoint_dir` | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig10_cov0 | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig5_cov0p01 |

## Chronological Linkage

- Previous W&B run: Run 046 [`po_geom_sig10_cov0`](../../wave_1/run_046_po_geom_sig10_cov0/).
- Next W&B run: Run 048 [`po_geom_sig7p5_cov0p003`](../run_048_po_geom_sig7p5_cov0p003/).
...
|---|---|
| W&B run files | `artifact/3087327211/wandb_manifest.json` (0 bytes), `requirements.txt` (3821 bytes), `wandb-metadata.json` (2636 bytes) |
| Logged artifacts | `run-az60m6mx-history:v0` (wandb-history) |
| API/group fetch caveats | none |

## Original Notes Preserved

**Command** (present-only geometry-sweep member; grid point lambda_sigreg=5.0 x lambda_cov=0.01 over the fixed-position/cosine present-only bottleneck; anchor/control = run 041 sig5/cov0):
...
python train.py --data ssv2 --steps 15000 --present-recon-only --recon-loss-mode cosine --lambda-recon 0.05 --lambda-var 0.5 --sigreg-warmup-steps 2000 --decoder-dim 512 --decoder-blocks 4 --n-c 32 --horizon-k 12 --log-every 50 --diag-every 500 \
  --lambda-sigreg 5.0 --lambda-cov 0.01 \
  --checkpoint-dir /workspace/ckpt/inv011_present_only_geometry_sweep/run_047_po_geom_sig5_cov0p01
```
