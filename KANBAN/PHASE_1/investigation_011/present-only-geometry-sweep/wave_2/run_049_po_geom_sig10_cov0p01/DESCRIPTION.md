...
**Current W&B run name:** `Investigation 11 · Present geometry · Isotropy 10, covariance 0.01`
...
**Command** (present-only geometry-sweep member; grid point lambda_sigreg=10.0 x lambda_cov=0.01 over the fixed-position/cosine present-only bottleneck; anchor/control = run 041 sig5/cov0):
...
python train.py --data ssv2 --steps 15000 --present-recon-only --recon-loss-mode cosine --lambda-recon 0.05 --lambda-var 0.5 --sigreg-warmup-steps 2000 --decoder-dim 512 --decoder-blocks 4 --n-c 32 --horizon-k 12 --log-every 50 --diag-every 500 \
  --lambda-sigreg 10.0 --lambda-cov 0.01 \
  --checkpoint-dir /workspace/ckpt/inv011_present_only_geometry_sweep/run_049_po_geom_sig10_cov0p01
```
