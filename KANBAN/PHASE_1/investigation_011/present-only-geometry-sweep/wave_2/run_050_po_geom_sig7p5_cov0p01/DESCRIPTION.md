# Run 050 - `po_geom_sig7p5_cov0p01`

**Current W&B run name:** `Investigation 11 · Present geometry · Isotropy 7.5, covariance 0.01`

<!-- AUTO-GENERATED-WANDB-KANBAN -->

**Investigation:** [investigation_011](../../../)  
**W&B:** `h5t89ezx` - https://wandb.ai/smahalanobis-uc-davis/hjepa-vwm/runs/h5t89ezx  
**State:** `crashed`  
**Created:** 2026-07-02T03:48:46Z  
**Last history step:** 14300  
...
Present-only geometry sweep wave 2 member: sigreg=7.5, cov=0.01.
...
| `agc_enabled` | true |
| `checkpoint_dir` | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig7p5_cov0p01 |

## Difference From Previous W&B Run

| Config key | Previous run | This run |
|---|---:|---:|
| `lambda_sigreg` | 10 | 7.5 |
| `checkpoint_dir` | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig10_cov0p01 | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig7p5_cov0p01 |

## Chronological Linkage

- Previous W&B run: Run 049 [`po_geom_sig10_cov0p01`](../run_049_po_geom_sig10_cov0p01/).
- Next W&B run: Run 051 [`po_geom_sig12p5_cov0p003`](../run_051_po_geom_sig12p5_cov0p003/).
...
|---|---|
| W&B run files | `artifact/3087326838/wandb_manifest.json` (0 bytes), `requirements.txt` (3821 bytes), `wandb-metadata.json` (2638 bytes) |
| Logged artifacts | `run-h5t89ezx-history:v0` (wandb-history) |
| API/group fetch caveats | none |

## Original Notes Preserved

**Command** (present-only geometry-sweep member; grid point lambda_sigreg=7.5 x lambda_cov=0.01 over the fixed-position/cosine present-only bottleneck; anchor/control = run 041 sig5/cov0):
...
python train.py --data ssv2 --steps 15000 --present-recon-only --recon-loss-mode cosine --lambda-recon 0.05 --lambda-var 0.5 --sigreg-warmup-steps 2000 --decoder-dim 512 --decoder-blocks 4 --n-c 32 --horizon-k 12 --log-every 50 --diag-every 500 \
  --lambda-sigreg 7.5 --lambda-cov 0.01 \
  --checkpoint-dir /workspace/ckpt/inv011_present_only_geometry_sweep/run_050_po_geom_sig7p5_cov0p01
```
