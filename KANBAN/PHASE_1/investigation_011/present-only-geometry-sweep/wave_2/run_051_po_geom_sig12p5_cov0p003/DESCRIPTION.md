# Run 051 - `po_geom_sig12p5_cov0p003`

**Current W&B run name:** `Investigation 11 · Present geometry · Isotropy 12.5, covariance 0.003`

<!-- AUTO-GENERATED-WANDB-KANBAN -->

**Investigation:** [investigation_011](../../../)  
**W&B:** `mtrviiab` - https://wandb.ai/smahalanobis-uc-davis/hjepa-vwm/runs/mtrviiab  
**State:** `crashed`  
**Created:** 2026-07-02T03:48:46Z  
**Last history step:** 14250  
...
Present-only geometry sweep wave 2 member: sigreg=12.5, cov=0.003.
...
| `agc_enabled` | true |
| `checkpoint_dir` | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig12p5_cov0p003 |

## Difference From Previous W&B Run

| Config key | Previous run | This run |
|---|---:|---:|
| `lambda_cov` | 0.01 | 0.003 |
| `lambda_sigreg` | 7.5 | 12.5 |
| `checkpoint_dir` | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig7p5_cov0p01 | /workspace/ckpt/inv011_present_only_geometry_sweep/po_geom_sig12p5_cov0p003 |

## Chronological Linkage

- Previous W&B run: Run 050 [`po_geom_sig7p5_cov0p01`](../run_050_po_geom_sig7p5_cov0p01/).
- Next W&B run: Run 052 [`ae_sharp_slots_recon_only`](../../../../investigation_012/run_052_ae_sharp_slots_recon_only/).
...
|---|---|
| W&B run files | `artifact/3087327368/wandb_manifest.json` (0 bytes), `requirements.txt` (3821 bytes), `wandb-metadata.json` (2642 bytes) |
| Logged artifacts | `run-mtrviiab-history:v0` (wandb-history) |
| API/group fetch caveats | none |

## Original Notes Preserved

**Command** (present-only geometry-sweep member; grid point lambda_sigreg=12.5 x lambda_cov=0.003 over the fixed-position/cosine present-only bottleneck; anchor/control = run 041 sig5/cov0):
...
python train.py --data ssv2 --steps 15000 --present-recon-only --recon-loss-mode cosine --lambda-recon 0.05 --lambda-var 0.5 --sigreg-warmup-steps 2000 --decoder-dim 512 --decoder-blocks 4 --n-c 32 --horizon-k 12 --log-every 50 --diag-every 500 \
  --lambda-sigreg 12.5 --lambda-cov 0.003 \
  --checkpoint-dir /workspace/ckpt/inv011_present_only_geometry_sweep/run_051_po_geom_sig12p5_cov0p003
```
