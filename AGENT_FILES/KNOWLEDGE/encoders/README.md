...
`encoders.py` provides pinned real V-JEPA2, SigLIP 2, and DINOv3 ViT-B/16 adapters on
`transformers==4.57.6`; SigLIP's vision-only 8x16x16x768 path has passed a real Mac/MPS
smoke, and DINO defaults to validated Hub commit
`5931719e67bbdb9737e363e781fb0c67687896bc`. Dataset identities include container frame
counts, exact resume consumes the saved sampler position, resource reports include CUDA-event
examples/frames/tokens rates, and frame encoders receive pre-concatenation frame rank/norm
diagnostics. The DINO weights remain gated, so a runtime still needs authorized Hugging Face
access even though the source revision is pinned. CUDA, real SSv2/EGO4D, stats, resource,
and paid-run evidence must be produced on the authenticated RunPod using `GUIDE_encoders.md`.
